...
function getColaByIdUser(idusuario) {
    return {
        query: "SELECT t.*, CONCAT_WS(' ', p.nombre, p.apellido) as nombrep, p.* FROM cola c JOIN ticket t ON c.id_ticket = t.id JOIN persona p ON t.id_persona = p.id WHERE c.id_usuario =  ? AND c.estado = 1 AND c.activo = 1",
        values: [idusuario],
    };
}
